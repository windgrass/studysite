...
if(DEBUG=="OFF") if(file_exists('install/index.php')) system_error('Contact <a href="http://www.nuno-silva.pt" target="_blank">Support</a>.',0);
...
function makepagelink($link, $page, $pages, $sub='') {
	$page_link = '<ul class="page_numbers">';
	if($page != 1) $page_link .= '<li><a href="'.$link.'&amp;'.$sub.'page=1"><i class="fa fa-angle-left"></i><i class="fa fa-angle-left"></i></a></li>';
	if($page >= 6) $page_link .= '<li><a href="'.$link.'&amp;'.$sub.'page='.($page-5).'"><i class="fa fa-angle-left"></i></a></li>';
	if($page+4 >= $pages) $pagex=$pages;
	else $pagex = $page+4;
	for($i=$page-4 ; $i<=$pagex ; $i++) {
		if($i<=0) $i=1;
		if($i==$page) $page_link .= '<li class="active"><a href="#">'.$i.'</a></li>';
		else $page_link .= '<li><a href="'.$link.'&amp;'.$sub.'page='.$i.'">'.$i.'</a></li>';
	}
	if(($pages-$page) >= 5) $page_link .= '<li><a href="'.$link.'&amp;'.$sub.'page='.($page+5).'"><i class="fa fa-angle-right"></i></a></li>';
	if($page != $pages) $page_link .= '<li><a href="'.$link.'&amp;'.$sub.'page='.$pages.'"><i class="fa fa-angle-right"></i><i class="fa fa-angle-right"></i></a></li>';
	$page_link .= '</ul>';
