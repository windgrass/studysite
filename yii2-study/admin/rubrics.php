...
if(isset($_POST['save'])) {
	...
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
		...
		if(checkforempty(Array('name'))) {
			safe_query("INSERT INTO ".PREFIX."news_rubrics ( rubric, color ) values( '".$_POST['name']."', '".$_POST['colors']."' ) ");
			$id=mysql_insert_id();
...
elseif(isset($_POST['saveedit'])) {
	...
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
		$pic = $_FILES['pic'];
		if(checkforempty(Array('name'))) {
			safe_query("UPDATE ".PREFIX."news_rubrics SET rubric='".$_POST['name']."', color='".$_POST['colors']."' WHERE rubricID='".$_POST['rubricID']."'");
...
if($action=="add") {
	...
	<form method="post" action="admincenter.php?site=rubrics" enctype="multipart/form-data">
  <table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover">
	...
	<tr>
      ...
      <td width="85%"><input class="color" name="colors" value="" /></td>
    </tr>
    <tr>
...
elseif($action=="edit") {
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover">
	...
	<tr>
      <td width="15%"><b>Color</b></td>
      <td width="85%"><input class="color" name="colors" value="'.getinput($ds['color']).'" /></td>
    </tr>
    <tr>
  ...
  echo'<table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover">
    <tr>
	  ...
	  <td width="55%" class="title"><b>Color</b></td>
  ...
  while($ds=mysql_fetch_array($ergebnis)) {
    if($i%2) { $td='td1'; }
	  ...
	  <td class="'.$td.'" align="center" style="vertical-align: middle"><div style="height: 20px; width: 20px; background: #'.$ds['color'].'"></div></td>
