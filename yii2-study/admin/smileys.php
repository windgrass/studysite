...
<tr>
      <td colspan="2" style="background-color: #4f5259; color: #FFFFFF">'.$_language->module['add_smiley'].'</td>
    </tr>
    <tr>
...
elseif($action=="edit") {
  ...
  echo'<form method="post" action="admincenter.php?site=smileys" enctype="multipart/form-data">
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover">
<tr>
      <td colspan="2" style="background-color: #4f5259; color: #FFFFFF">'.$_language->module['edit_smiley'].'</td>
    </tr>
    <tr>
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover">
    ...
    while($smileys = mysql_fetch_array($ds)) {
      ...
      echo'<tr>
        ...
        <td class="'.$td.'" >'.$smileys['pattern'].'</td>
        <td class="'.$td.'" ><input type="button" onclick="MM_goToURL(\'parent\',\'admincenter.php?site=smileys&amp;action=edit&amp;smileyID='.$smileys['smileyID'].'\');return document.MM_returnValue" value="'.$_language->module['edit'].'" />
