...
$filepath = "../images/bannerrotation4/";
...
if($action=="add") {
  ...
  echo'<form method="post" action="admincenter.php?site=bannerrotation4" enctype="multipart/form-data">
  <table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover">
    ...
    <tr>
      ...
      <td width="85%"><input name="banner" type="file" size="40" /> <small>(Image width size: 280px)</small></td>
    </tr>
    <tr>
...
elseif($action=="edit") {
	...
	$ds=mysql_fetch_array(safe_query("SELECT * FROM ".PREFIX."bannerrotation4 WHERE bannerID='".$_GET["bannerID"]."'"));
	if(file_exists($filepath.$ds['bannerID'].'.gif'))	$pic='<img src="../images/bannerrotation4/'.$ds['bannerID'].'.gif" border="0" alt="'.$ds['banner'].'" />';
	elseif(file_exists($filepath.$ds['bannerID'].'.jpg'))	$pic='<img src="../images/bannerrotation4/'.$ds['bannerID'].'.jpg" border="0" alt="'.$ds['banner'].'" />';
	elseif(file_exists($filepath.$ds['bannerID'].'.png'))	$pic='<img src="../images/bannerrotation4/'.$ds['bannerID'].'.png" border="0" alt="'.$ds['banner'].'" />';
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover">
    ...
    <tr>
      ...
      <td><input name="banner" type="file" size="40" /> <small>(Image width size: 280px)</small></td>
    </tr>
    <tr>
      <td><b>'.$_language->module['banner_name'].'</b></td>
	...
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
		if($bannername AND $bannerurl AND $banner) {
			...
			if($file_ext==".gif" OR $file_ext==".jpg" OR $file_ext==".png") {
				safe_query("INSERT INTO ".PREFIX."bannerrotation4 (bannerID, bannername, bannerurl, displayed, date) values('', '".$bannername."', '".$bannerurl."', '".$displayed."', '".time()."')");
				$id=mysql_insert_id();
				if($banner['name'] != "") {
					...
					if(safe_query("UPDATE ".PREFIX."bannerrotation4 SET banner='".$file."' WHERE bannerID='".$id."'")) {
						redirect("admincenter.php?site=bannerrotation4","",0);
					} else {
						redirect("admincenter.php?site=bannerrotation4","",0);
					}
				}
...
elseif(isset($_POST["saveedit"])) {
	...
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {	
		if($banner AND $bannername AND $bannerurl) {
			if(stristr($bannerurl,'http://')) $bannerurl=$bannerurl;
			else $bannerurl=$bannerurl;
	
			if($banner['name']=="") {
				if(safe_query("UPDATE ".PREFIX."bannerrotation4 SET bannername='".$bannername."', bannerurl='".$bannerurl."', displayed='".$displayed."' WHERE bannerID='".$_POST["bannerID"]."'"))
					redirect("admincenter.php?site=bannerrotation4","",0);			
	    	} else {
				$file_ext=strtolower(mb_substr($banner['name'], strrpos($banner['name'], ".")));
				if($file_ext==".gif" OR $file_ext==".jpg" OR $file_ext==".png") {
					...
					if(safe_query("UPDATE ".PREFIX."bannerrotation4 SET banner='".$file."', bannername='".$bannername."', bannerurl='".$bannerurl."', displayed='".$displayed."' WHERE bannerID='".$_POST["bannerID"]."'")) {
						redirect("admincenter.php?site=bannerrotation4","",0);
					}
				} else echo'<b>'.$_language->module['format_incorrect'].'</b><br /><br /><a href="javascript:history.back()">&laquo; '.$_language->module['back'].'</a>';
			}
...
elseif(isset($_GET["delete"])) {
	...
	if($CAPCLASS->check_captcha(0, $_GET['captcha_hash'])) {
		if(safe_query("DELETE FROM ".PREFIX."bannerrotation4 WHERE bannerID='".$_GET["bannerID"]."'")) {
  ...
  echo'<input type="button" onclick="MM_goToURL(\'parent\',\'admincenter.php?site=bannerrotation4&amp;action=add\');return document.MM_returnValue" value="'.$_language->module['new_banner'].'" /><br /><br />';
  
  echo'<form method="post" action="admincenter.php?site=bannerrotation4">
  <table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover">
    <tr>
      <td width="20%" class="title"><b>'.$_language->module['banner'].' name</b></td>
  ...
  $CAPCLASS = new Captcha;
	...
	$qry=safe_query("SELECT * FROM ".PREFIX."bannerrotation4 ORDER BY bannerID");
    ...
    while($ds = mysql_fetch_array($qry)) {
      if($i%2) { $td='td1'; }
        ...
        <td class="'.$td.'"><img src="../images/bannerrotation4/'.getinput($ds['banner']).'" width="150px" border="0" /></td>
		<td class="'.$td.'">'.$bannerurl.'</td>
        <td class="'.$td.'">'.$ds['hits'].' ('.$perday.')</td>
        <td class="'.$td.'" align="center">'.$displayed.'</td>
        <td class="'.$td.'" align="center"><input type="button" onclick="MM_goToURL(\'parent\',\'admincenter.php?site=bannerrotation4&amp;action=edit&amp;bannerID='.$ds['bannerID'].'\');return document.MM_returnValue" value="'.$_language->module['edit'].'" />
        <input type="button" onclick="MM_confirm(\''.$_language->module['really_delete'].'\', \'admincenter.php?site=bannerrotation4&amp;delete=true&amp;bannerID='.$ds['bannerID'].'&amp;captcha_hash='.$hash.'\')" value="'.$_language->module['delete'].'" /></td>
      </tr>';
