	...
	if($autoresize>0) {
		...
		foreach($imgtags as $teil) {
			...
			if($autoresize == 1) {
				...
				$n = str_replace(' ', '', $n);
				$content = preg_replace('#\[img\]'.preg_quote($teil[2],"#").'\[/img\]#si', '<a href="'.fixJavaEvents($teil[2]).'" class="lightbox"><img src="'.fixJavaEvents($teil[2]).'" id="ws_image_'.$n.'" border="0" onload="checkSize(\''.$n.'\', '.$picsize_l.', '.$picsize_h.')" alt="'.fixJavaEvents($teil[2]).'" style="max-width: '.($picsize_l+1).'px; max-height: '.($picsize_h+1).'px;" /></a><div id="ws_imagediv_'.$n.'" style="display:none;"></div>', $content, 1);
			}
		}
	}
...
function quotereplace($content) {
	...
	$content = preg_replace("#\[quote\]#s", '<blockquote class="alt"><p>', $content, 10);
	$content = preg_replace("#\[/quote\]#s", '</p></blockquote>', $content, 10);
