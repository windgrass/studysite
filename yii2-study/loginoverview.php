...
if($userID && !isset($_GET['userID']) && !isset($_POST['userID'])) {
	...
	$username='<a href="index.php?site=user&amp;id='.$userID.'">'.getnickname($userID).'</a>';
				...
				while($ds=mysql_fetch_array($ergebnis)) {
					...
					if(mysql_num_rows($anmeldung)) {
						...
						while ($da = mysql_fetch_array($anmeldung)) {
							...
							if($i>1) $players.=', <a href="index.php?site=user&amp;id='.$da['userID'].'"><font color="'.$fontcolor.'">'.strip_tags(stripslashes(getnickname($da['userID']))).'</font></a>';
							else $players.='<a href="index.php?site=user&amp;id='.$da['userID'].'"><font color="'.$fontcolor.'">'.strip_tags(stripslashes(getnickname($da['userID']))).'</font></a>';
							$i++;
						}
