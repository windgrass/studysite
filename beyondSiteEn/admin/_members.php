...
if(isset($_POST['saveedit'])) {
	...
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
		...
		$shopadmin = isset($_POST['shopadmin']);
		...
		if($userID != $id OR issuperadmin($userID)) {
			...
			safe_query("UPDATE ".PREFIX."user_groups SET news='$newsadmin',
													  ...
													  files='$fileadmin',
	                          						  cash='$cashadmin',
													  shop='$shopadmin' WHERE userID='".$id."'");
			//remove from mods
			if($moderator == false){
...
if(isset($_GET['action']) and $_GET['action'] == "edit") {
  ...
  eval ("\$addflags = \"".gettemplate("flags_admin", "html", "admin")."\";");
	...
	//shop
	if(isshopadmin($id)) $shop='<input type="checkbox" name="shopadmin" value="1" onmouseover="showWMTT(\'id14\')" onmouseout="hideWMTT()" checked="checked" />';
	else $shop='<input type="checkbox" name="shopadmin" value="1" onmouseover="showWMTT(\'id14\')" onmouseout="hideWMTT()" />';
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3">
	...
	<tr>
      <td>'.$shop.' '.$_language->module['shop_admin'].'</td>
    </tr>';
