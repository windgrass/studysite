...
if(isset($_POST['addteam'])) {
 	$CAPCLASS = new Captcha;
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
		$anz = mysql_num_rows(safe_query("SELECT userID FROM ".PREFIX."teams_members WHERE teamID='".$_POST['team']."' AND userID='".$_POST['id']."'"));
		if(!$anz){
			safe_query("INSERT INTO ".PREFIX."teams_members (teamID, userID, position, activity, sort) values('".$_POST['team']."', '".$_POST['id']."', '".$_POST['position']."', '".$_POST['activity']."', '1')");	
		}
	  	else{
			echo $_language->module['user_exists'];
		}
			...
			if($info[0] == 100 && $info[1] == 100) {
				...
				if($pic != "") {
					...
					if(unlink($filepath.$avatar['name'].".tmp")) {
						$error = 'Upload Failed. Invalid Format. Only gif,jpg and png extentions allowed.';
						die('ERROR: '.$error.'<br /><br /><input type="button" onclick="javascript:history.back()" value="'.$_language->module['back'].'" />');
					}
					else {
                                                       $error = 'Upload Failed. Avatar size is not equal to 58px x 58px ';
						die('ERROR: '.$error.'<br /><br /><input type="button" onclick="javascript:history.back()" value="'.$_language->module['back'].'" />');
					}
				}
			}
				...
				if($pic != "") {
					...
					if(unlink($filepath.$userpic['name'].".tmp")) {
						...
						die('ERROR: '.$error.'<br /><br /><input type="button" onclick="javascript:history.back()" value="'.$_language->module['back'].'" />');
                                                       ...
                                                       $error = 'Upload Failed. Picture size is not equal to 80px x 80px ';
						die('ERROR: '.$error.'<br /><br /><input type="button" onclick="javascript:history.back()" value="'.$_language->module['back'].'" />');
					}
				}
			}
...
elseif($action=="ban") {
    ...
    <tr>
      <td colspan="2" style="background-color: #4f5259; color: #FFFFFF">'.$_language->module['ban_user'].'</td>
    </tr>
			  <tr>
...
elseif($action=="addtoclan") {
  ...
  echo'<form method="post" action="admincenter.php?site=users&amp;page='.(int)$_GET['page'].'&amp;type='.getforminput($_GET['type']).'&amp;sort='.$_GET['sort'].'">
	<table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover">
    <tr>
      <td colspan="2" style="background-color: #4f5259; color: #FFFFFF">'.$_language->module['add_to_clan'].'</td>
    </tr>
    <tr>
      <td width="15%"><b>'.$_language->module['nickname'].'</b></td>
      <td width="85%">'.$nickname.'</td>
...
elseif($action=="addtoteam") {

function getsteams() {	
	$teams='';
	$ergebni = safe_query("SELECT DISTINCT t.* FROM `".PREFIX."songs` as t, `".PREFIX."user` as l WHERE t.game=l.game ORDER BY t.name");
	while($ds = mysql_fetch_array($ergebni)) {
	$teams .= '<option value="'.$ds['id'].'">'.$ds['name'].' ('.getgamename($ds['game']).')</option>';
	}
	return $teams;
}
  ...
  $id = $_GET['id'];
  	...
  	$gamee='';
	$ergebnia = safe_query("SELECT game FROM `".PREFIX."user` WHERE userID='".$id."'");
	while($ds = mysql_fetch_array($ergebnia)) {
	$gamee = getgamename($ds['game']);
	}

  $nickname=getnickname($id);
  $CAPCLASS = new Captcha;
  if($gamee == '') $teams = $teams_first;
  else $teams = getsteams();
  $CAPCLASS->create_transaction();
  $hash = $CAPCLASS->get_hash();
  ...
  echo'<form method="post" action="admincenter.php?site=users&amp;page='.(int)$_GET['page'].'&amp;type='.getforminput($_GET['type']).'&amp;sort='.$_GET['sort'].'">
	<table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover">
    <tr>
      <td colspan="2" style="background-color: #4f5259; color: #FFFFFF">Add to Team</td>
    </tr>
    <tr>
      <td width="15%"><b>'.$_language->module['nickname'].'</b></td>
      <td width="85%">'.$nickname.'</td>
	...
	<tr>
      ...
      <td width="85%">'.$gamee.'</td>
    </tr>
    <tr>
      <td><b>Team</b></td>
      <td><select name="team">'.$teams.'</select></td>
    </tr>
    <tr>
      <td><b>'.$_language->module['position'].'</b></td>
      <td><input type="text" name="position" size="60" /></td>
    ...
    <tr>
      <td><input type="hidden" name="captcha_hash" value="'.$hash.'" /><input type="hidden" name="id" value="'.$id.'" /></td>
      <td><br /><input type="submit" name="addteam" value="Add to Team" /></td>
    </tr>
  </table>
  </form>';
}
...
elseif($action=="adduser") {
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover">
    <tr>
      <td colspan="2" style="background-color: #4f5259; color: #FFFFFF">'.$_language->module['add_new_user'].'</td>
    </tr>
    <tr>
...
elseif($action=="profile") {
  ...
  function getgamelista() {
		...
		while($game = mysql_fetch_array($games)) {
			...
			$gamelist.='<option '.$checked.' value="'.$game['tag'].'">'.$game['name'].'</option>';
		}
	return $gamelist;
}
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover">
    <tr>
      <td colspan="2" style="background-color: #4f5259; color: #FFFFFF">'.$_language->module['edit_profile'].'</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><input name="avatar" type="file" size="40" /> <small>(must be equal to 100px x 100px)</small></td>
    </tr>
    <tr>
      <td></td>
    ...
    <tr>
      <td><b>'.$viewpic.'</b></td>
      <td><input name="userpic" type="file" size="40" /> <small>(must be equal to 150px x 150px)</small></td>
    </tr>
    <tr>
      <td></td>
	  				...
	  				IF( 	(SELECT userID FROM ".PREFIX."user_groups WHERE userID=u.userID AND (page='1' OR forum='1' OR user='1' OR news='1' OR clanwars='1' OR feedback='1' OR super='1' OR gallery='1' OR cash='1' OR files='1') LIMIT 0,1) =u.userID,2, 
	  					IF( 	(SELECT userID FROM ".PREFIX."user_groups WHERE userID=u.userID AND moderator='1' LIMIT 0,1) = u.userID, 3, 
	  						IF( 	(SELECT userID FROM ".PREFIX."squads_members WHERE userID=u.userID LIMIT 0,1) = u.userID,4,5 ) 
								IF( 	(SELECT userID FROM ".PREFIX."teams_members WHERE userID=u.userID LIMIT 0,1) = u.userID,6,7 ) 
	  					) 
	  				) 
	  			)";
	  	$status = true;
	...
	if($anz) {
		...
		echo'<div style="float: left; position: relative; z-index: 200; height: 100px; width: 100%;"><table width="100%" border="0" cellspacing="1" cellpadding="3" class="table">
		      <tr>
      ...
      <tr>
        ...
        <div id="searchresult" style="width: auto; max-width: 250px; display:none; border:1px solid black; background-color:#DDDDDD; padding:2px;"></div></td>
      </tr>
    </table></div>';

		echo'<br />
    <table width="100%" border="0" cellspacing="1" cellpadding="3" class="table table-hover" style="position: relative; z-index: 100;">
      <tr>
        ...
        <td width="17%" class="title" align="center"><a href="admincenter.php?site=users&amp;type='.$type.'&amp;sort=status&amp;page='.$page.'&amp;type='.$type.'&amp;search='.$search.'"><b>'.$_language->module['status'].'</b></a></td>
        <td width="12%" class="title" align="center"><b>'.$_language->module['ban_status'].'</b></td>
        <td width="33%" class="title" align="center" colspan="2"><b>'.$_language->module['actions'].'</b></td>
      </tr>';
      ...
      if($i%2) { $td='td1'; }
        ...
        <td class="'.$td.'"><a href="../index.php?site=user&amp;id='.$id.'" target="_blank">'.strip_tags(stripslashes($nickname)).'</a></td>
